from .exceptions import ModeException
from .decorators import validate_params
from .config import PAYU_CONFIGS
import hashlib
import logging
import requests
...
class Payu:
    ...
    def __init__(self, merchant_key, merchant_salt, mode):
    ...
    def verify_payment(self, *args, **kwargs):
        ...
        """
        if len(kwargs.get('txnid')) == 0:
            raise KeyError(
                " Requested data doesn't contains required field: txnid")
        kwargs['command'] = "verify_payment"
        kwargs['var1'] = kwargs['txnid']
